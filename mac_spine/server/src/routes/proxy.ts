...
export async function handleProxy(
  ...
  if (!body.action || typeof body.action !== 'string') {
    return errorResponse(400, 'MISSING_FIELDS', 'Missing required field: action', corsHeaders);
  }
  if (!body.record_id || typeof body.record_id !== 'string') {
    return errorResponse(400, 'MISSING_FIELDS', 'Missing required field: record_id', corsHeaders);
  }

  const signature = request.headers.get('X-Request-Signature');
  if (!signature) {
    return errorResponse(400, 'MISSING_FIELDS', 'Missing required header: X-Request-Signature', corsHeaders);
  }
  ...
  if (!valid) {
    ...
    return errorResponse(403, 'HMAC_INVALID', 'Signature verification failed', corsHeaders);
  }

  // ---- 4. Action whitelist ----
  if (body.action !== 'upload_data') {
    log(body.action, body.record_id, 400);
    return errorResponse(400, 'UNKNOWN_ACTION', `Unknown action: ${body.action}`, corsHeaders);
  }
  ...
  try {
    ...
    if (!exists) {
      ...
      return errorResponse(404, 'PARTICIPANT_NOT_FOUND', 'Record ID not found in REDCap', corsHeaders);
    }
  } catch (err) {
    log(body.action, body.record_id, 502, {
      error: err instanceof Error ? err.message : 'Unknown error',
    });
    return errorResponse(502, 'REDCAP_ERROR', 'Failed to verify participant in REDCap', corsHeaders);
  }
  ...
  for (const record of records) {
    ...
    try {
      ...
      return errorResponse(502, 'REDCAP_ERROR', 'Deduplication check failed', corsHeaders);
    }
    ...
    try {
      ...
      return errorResponse(502, 'REDCAP_REJECTED', 'REDCap rejected the import', corsHeaders);
    }
  }
