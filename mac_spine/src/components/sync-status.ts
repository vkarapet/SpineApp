...
export function createSyncStatus(config: SyncStatusConfig): HTMLElement {
  ...
  if (pendingCount === 0) {
    icon.innerHTML = `<svg width="20" height="20" viewBox="-1 -1 26 26" fill="none" stroke="#0E5B3D" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>`;
    icon.setAttribute('aria-label', 'All synced');
  } else {
    icon.innerHTML = `<svg width="20" height="20" viewBox="-1 -1 26 26" fill="none" stroke="#FDBF57" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><circle cx="12" cy="16" r=".5" fill="#FDBF57"/></svg>`;
